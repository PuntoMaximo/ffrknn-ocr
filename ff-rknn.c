                       ...
                       int dst_format, int frameSize, char *buf, char *viraddr)
{
    ...
    rga_info_t dst;
    int ret;
    hStride = (src_Height + 15) & (~15);
    wStride = (src_Width + 15) & (~15);
    ...
    rga_set_rect(&src.rect, 0, 0, src_Width, src_Height, wStride, hStride,
                 src_format);
    rga_set_rect(&dst.rect, 0, 0, dwStride, dhStride, dwStride, dhStride,
                 dst_format);
                              ...
                              AVPacket *pkt)
{
    ...
    while (ret >= 0) {
        ...
        if (desc && layer) {
            ...
            std::cout << drm_format << "\n";
            src_format = (RgaSURF_FORMAT)drm_get_rgaformat(DRM_FORMAT_YUV420);
